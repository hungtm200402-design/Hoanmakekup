...
export function ServiceCard({ slug, title, price, image, desc, duration }: { slug: string; title: string; price: string; image: string; icon?: string; desc?: string; duration?: string }) {
  return (
    <article className="overflow-hidden rounded-[10px] border border-[#f6cfd8] bg-white text-center shadow-[0_10px_26px_rgba(217,46,85,0.06)]">
      <div className="h-[292px] bg-gradient-to-br from-[#fff4f6] to-white max-[760px]:h-[240px]">
        {image ? (
          <img className="h-full w-full object-cover" src={image} alt={title} />
        ) : (
          <div className="grid h-full place-items-center text-[12px] font-semibold uppercase text-brand-red/55">Khung ảnh dịch vụ</div>
        )}
      </div>
